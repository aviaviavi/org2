...
- *Publish*: =publish document= produces a disclosure-safe web bundle or Google Doc from one document or subtree; =export html= and project =publish= derive ordinary HTML, while =export beamer= derives reviewable LaTeX or a compiled presentation PDF from an Org/Org2 talk. Publishing previews by default and never grants access to the source corpus.
