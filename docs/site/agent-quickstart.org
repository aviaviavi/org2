...
Use =org2 checkbox --file FILE --line N= to preview a three-state list-checkbox cycle, or =checkbox set --status checked= to choose a state. Add =--apply= to write; carry the JSON preview revision with =--if-revision= when applying later.
...
- *Keep a host available*: =org2 server= runs the existing chat relay and scheduler without a desktop window on macOS. Pair iOS over Tailscale, and use preview-first =server assign --host-ref HOST= to choose one corpus scheduler owner. See [[file:headless-server.org][Headless server setup]].
