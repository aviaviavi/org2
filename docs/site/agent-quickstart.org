...
- *Scale recurring account work*: =ledger create/update/event/list/show= stores one guarded canonical account per file with stable identity keys and idempotent history. Account Outreach uses =notes/account-outreach/accounts/=. Approval events link the canonical run decision, sent outreach requires an external receipt, and =ledger list --eligible= excludes pending or not-yet-executed approved work plus recently contacted accounts.
