...
- *Delegate durable work*: =run create/list/show/start/resume/cancel/fork= keeps goals, cited context, plans, assignments, runtime metadata, artifacts, approvals, validation, and event history in =.org2/runs/*.org2=. =org2 approvals= is the unified pending-decision queue: every item carries a =queueId= and a SHA-256 fingerprint of the exact material reviewed. Native queue IDs are stable; legacy heading IDs bind the currently cited file and heading identity, with a line-based locator only as a provisional fallback. Decide either kind through =org2 approval decide QUEUE_ID --decision ... --actor ... --expected-fingerprint ...=; changed material or a moved provisional identity must be refreshed instead of receiving the old decision. Run approvals remain canonical in their run record and must not be duplicated in a heading. External-action and high-impact approvals stay disabled until exact review material is attached. Use =run runtime= for observable provider/model and usage fields. If work cannot continue, =run block ID --reason "Specific question or next action"= records an actionable clarification; a generic reasonless blocked state is rejected. Runs with =review-required= artifacts remain open until =run artifact-review RUN_ID ARTIFACT_ID --status reviewed|rejected --actor NAME= records the human decision in both the run and a linked Org artifact. When a person confirms that a blocked outcome was already achieved elsewhere, =run complete-external RUN_ID --summary "Where or how it was completed" --actor NAME= closes that run with an explicit audit event; declared approval requirements still must be satisfied unless their guarded step was explicitly skipped because no effect occurred.
- *Teach and replay a process*: =workflow save/run/triggers/package= converts a completed run into a versioned recipe with inputs, capabilities, outputs, checks, and approval boundaries. Workflow approval declarations materialize as durable run requirements. Bind a concrete request with =run approval-request RUN_ID --requirement REQUIREMENT_ID ...=; Org2 will not start or complete a guarded step, or complete the run, until the current request for each applicable requirement is approved.
...
Install the checked-out integration with =openclaw plugins install --link ./integrations/openclaw=. The =org2-lifecycle= plugin must be enabled, permitted to use its typed conversation hooks, and configured with the target =corpusDir=. Verify the live Gateway surface with =openclaw plugins inspect org2-lifecycle --runtime --json= after restarting the Gateway. The adapter is a single-corpus write boundary: Mac workflow requests include the active portable corpus ID and fail before execution if it differs from the configured corpus. Ordinary conversation and personal TODOs stay out of Agent Work; substantial main-agent work, subagents, and cron executions receive correlated durable run records. Workflow turns that request approval remain open, and a Mac approval continues the same durable run in its correlated chat session. Gmail drafts are sent only through the typed =org2_gmail_draft_send= tool. It resolves account aliases to the canonical provider mailbox, binds the exact headers, MIME bodies, attachments, thread identity, RFC822 raw digest, and runtime target to the native approval, then reserves that one effect. The provider receives the exact reviewed RFC822 snapshot rather than a mutable draft ID; its message and thread receipt must match, and the provider draft is intentionally retained so a concurrent edit cannot be deleted. Recognizable direct shell or Gmail API sends are blocked; deployments must also keep Gmail credentials and egress away from arbitrary general-purpose execution because command inspection cannot prove opaque code is mail-free.
