...
- *Delegate durable work*: =run create/list/show/start/resume/cancel/fork= keeps goals, cited context, plans, assignments, artifacts, approvals, validation, and event history in =.org2/runs/*.org2=. If work cannot continue, =run block ID --reason "Specific question or next action"= records an actionable clarification; a generic reasonless blocked state is rejected.
...
The durable record is ordinary Org2 text under =.org2/runs/=. Its human-readable sections and versioned JSON machine-state block describe the same run. Completion requires a concise =--summary= written for the person reviewing the result; repeat =--highlight= and =--next-action= when those details matter. Workflow packages use the same pattern under =.org2/workflows/=. Apps and agents may index these files, but they are not allowed to replace them with private state.
