...
- *Edit property views*: =property-view list/query/save/edit= shares the Mac Property Views builder semantics. Portable =views/ID.org2-view.json= definitions select note/heading rows, columns, filters, sort and grouping. Queries return inherited values, source locations and revisions; preview source edits and carry =--if-revision= before =--apply=.
