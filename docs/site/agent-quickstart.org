...
- *Delegate durable work*: =run create/list/show/start/resume/cancel/fork= keeps goals, cited context, plans, assignments, runtime metadata, artifacts, approvals, validation, and event history in =.org2/runs/*.org2=. Use =run runtime= for observable provider/model and usage fields. If work cannot continue, =run block ID --reason "Specific question or next action"= records an actionable clarification; a generic reasonless blocked state is rejected.
