...
TODO vocabularies belong in file-local =#+TODO:=, =#+SEQ_TODO:=, or =#+TYP_TODO:= declarations. Respect =|= terminal states and use =todo set --keyword STATE= for exact custom state names; inspect the preview before applying.
