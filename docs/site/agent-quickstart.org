...
- *Delegate durable work*: =run create/list/show/start/resume/cancel/fork= keeps goals, cited context, plans, assignments, artifacts, approvals, validation, and event history in =.org2/runs/*.org2=.
...
The durable record is ordinary Org2 text under =.org2/runs/=. Its human-readable sections and versioned JSON machine-state block describe the same run. Workflow packages use the same pattern under =.org2/workflows/=. Apps and agents may index these files, but they are not allowed to replace them with private state.
