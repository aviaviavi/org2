...
Use =org2 checkbox --file FILE --line N= to preview a three-state list-checkbox cycle, or =checkbox set --status checked= to choose a state. Use =checkbox fix-cookies --file FILE= to preview recalculation of stale =[n/m]= and =[p%]= cookies against their innermost heading totals. Add =--apply= to write; carry the JSON preview revision with =--if-revision= when applying later.
...
For a Codex machine reachable through SSH, add a *Managed Remote Codex* destination instead of maintaining a WebSocket tunnel. The destination stores the existing Codex or =~/.ssh/config= host alias and the workspace path on that machine, starts Codex's managed app-server daemon, and attaches to its private Unix socket through SSH. ChatGPT authentication remains owned by Codex; Org2 stores neither that credential nor a bearer token. If the Mac sleeps or the SSH stream drops after a turn starts, the remote daemon keeps owning the turn. OpenOrg reconnects and reads that exact turn after wake instead of submitting the message again. Client-hosted workspace tools still wait for the Mac to become available.
