...
The GitHub Actions workflow *OpenOrg macOS correctness and performance* runs the correctness suite before the optimized performance gates and records its Xcode, Swift, and SDK versions. A compilation or correctness failure prevents benchmarks from running; inspect the failed step before treating the workflow result as a measured slowdown. Performance thresholds remain enforced in the release-mode step.
