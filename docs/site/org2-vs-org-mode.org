...
The Mac app combines agenda planning, files and knowledge navigation, rendered reading, source editing, meetings, OpenClaw chat, data notebooks, interactive charts, and *Runs & Review*. These surfaces still write ordinary files and call the shared runtime.
...
Org2 treats cited context, durable runs, outputs, validation, clarification, approval, and reusable workflows as part of the project. The current OpenClaw plugin records substantial agent work into the Run Center, and the Mac app exposes the review loop.
