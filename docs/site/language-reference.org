...
=org2 query-data= reads these blocks, creates DuckDB views for =csv=, =parquet=, =json=, URL-backed sources, or named org2 table datasets, runs a selected SQL result block through the local DuckDB CLI, and materializes the returned rows as a named org table or JSON envelope:
