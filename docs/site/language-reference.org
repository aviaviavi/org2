...
For the normative draft standard, start with [[https://github.com/aviaviavi/org2/blob/main/spec/v0/SPEC.org][SPEC.org]]. The standard is independently implementable: its surface grammar, contextual parsing algorithms, canonical AST schema, and executable conformance cases are all public.

* Spec map

Use this mapping when deciding where behavior is defined:

- Language overview and construct requirements: [[https://github.com/aviaviavi/org2/blob/main/spec/v0/SPEC.org][SPEC.org]]
- Normative surface grammar: [[https://github.com/aviaviavi/org2/blob/main/spec/v0/GRAMMAR.ebnf][GRAMMAR.ebnf]]
- Parsing precedence, grouping, and AST construction: [[https://github.com/aviaviavi/org2/blob/main/spec/v0/PARSING.org][PARSING.org]]
- Canonical AST and executable conformance: [[https://github.com/aviaviavi/org2/blob/main/spec/v0/canonical-ast.schema.json][canonical-ast.schema.json]] and [[https://github.com/aviaviavi/org2/blob/main/spec/v0/parsing-cases.json][parsing-cases.json]]
...
Corpus defaults live in =org2.json= under =todo.sequences=, for example ={"todo":{"sequences":["TODO WAITING | DONE SKIPPED"]}}=. OpenOrg exposes these under Settings → Workspace → TODO States. Each corpus sequence needs an active state and a terminal state; state names must be unique across the corpus sequences. An empty array restores built-in defaults.

Files may also declare their own Org-compatible vocabulary with =#+TODO:=, =#+SEQ_TODO:=, or =#+TYP_TODO:=. Fast-selection suffixes such as =WAITING(w)= are recognized and omitted from the stored headline state:

#+begin_src org
#+TODO: IDEA(i) WAITING(w) | SHIPPED(s)
* WAITING Publish the package
#+end_src

File-local declarations replace the corpus sequences for that file. Without a file-local declaration, parsing, editing, agendas, and compiled context use the nearest corpus configuration.

States before =|= are active; states after it are terminal (no further action required). Without =|=, the last keyword is terminal. Multiple declarations define independent sequences. Keywords retain their spelling and case. Built-in states remain available. Declarations inside source/example blocks and drawers do not change the workflow.

Custom states work in TODO mutations, agenda filtering, and OpenOrg heading controls. Terminal states do not become overdue tasks; reopening clears =CLOSED=. Shortcut and logging annotations are accepted, but their Emacs-specific interactive prompts/keybindings are not implemented.
...
Priority is stored as the dedicated =priority= headline field. Org's =COMMENT= headline marker is stored as =commented: true= while all source bytes still round-trip unchanged.
...
- Ordered markers: =1.=, =1)=
- Checkboxes: =[ ]=, =[X]=, =[x]=, =[-]=
- Cycle a list checkbox with =org2 checkbox --file FILE --line N --apply=: =[ ]= → =[-]= → =[X]= → =[ ]=. The CLI and LSP code action change only the selected marker; see [[file:tooling-reference.org][Tooling reference]].
- Checkbox progress cookies: =[n/m]= and =[p%]= may appear in headings or list items; =[/]= and =[%]= are empty placeholders. Compile output and lint compare each cookie with its innermost containing heading total. Checked items count as completed; unchecked and indeterminate items count only toward the total. =org2 checkbox fix-cookies --file FILE --apply= writes corrected values, while =org2 checkbox cycle --file FILE --line N --fix-cookies --apply= changes a marker and refreshes cookies atomically.
- Nested lists via indentation.
- Counter cookies such as =[@3]= are stored separately from the written ordered-list marker.
- Descriptive items use =term :: definition= and render as description lists.
...
- Angle links: =<mailto:hello@example.com>=
- Targets: =<<name>>= and radio targets =<<<name>>>=
...
Org2 also parses the everyday publishing and scholarly inline objects used by Org:

- Footnote references: =[fn:label]=, =[fn:label:inline definition]=, and =[fn::anonymous definition]=
- Citations: =[cite:@key]= and multi-reference/style forms such as =[cite/t:@doe; see @roe p. 4]=
- Entities: =\alpha=, =\copy=, =\rarr=, and other common Org entities
- LaTeX fragments: =$x_2$=, =$$x = y$$=, =\(...\)=, and =\[...\]=
- Export snippets: =@@html:<mark>highlighted</mark>@@=
- Subscripts and superscripts: =H_2O= and =x^{2}=
- Explicit line breaks: two backslashes at the end of a line

These are first-class canonical AST objects and render in the CLI, published HTML, and Mac/iOS document views. Export snippets are emitted as raw HTML only by the publishing renderer; app previews escape their payload.

Footnote definitions use the ordinary Org form:

#+begin_src org
The claim has a source.[fn:source]

[fn:source] Supporting detail.
#+end_src
...
- Arbitrary special blocks: =#+begin_NAME ... #+end_NAME=
- Dynamic blocks: =#+begin: NAME ... #+end:= (parsed and preserved; Org2 does not execute dynamic-block writers)
- LaTeX environments: =\begin{equation} ... \end{equation}=
- Fixed-width lines beginning with a colon followed by one space
- Horizontal rules of five or more hyphens
- Diary sexps beginning with =%%(=

** Syntactic sugar / aliases

OpenOrg accepts a friendlier fenced-code typing alias for source blocks:
...
New =.org= documents use the ordinary Org form on disk. OpenOrg, file-based CLI formatting, the LSP, and editor integrations rewrite fences to =#+begin_src= / =#+end_src= and backtick inline code to ordinary Org inline markup whenever edited =.org= source is persisted or formatted. This syntax canonicalization still occurs when general format-on-save is disabled: the input remains quick to type, while persisted =.org= files stay valid Org syntax. Existing =.org2= files and the default lossless parser/printer preserve their original spelling.

File-based CLI formatting selects the profile automatically from the extension. Stdin has no filename, so buffer-oriented callers select it explicitly:
...
- Table content is parsed and preserved. Each row retains compatible raw =cells= strings and also exposes parallel structured =contents= inline objects.

Org table formulas are attached to their preceding table as structured =TableFormulaLine= nodes and can be recalculated safely:
...
=org2 table recalculate --file report.org= previews the changed cells; add =--apply= to write them atomically. The Mac rendered view evaluates supported formulas for display and offers the same explicit recalculation action for editable files. HTML export uses the shared evaluator without rewriting source.

The safe formula subset includes field, row, column, and rectangular-range assignments; absolute and relative =@row$column= references; A1 references; arithmetic, comparisons, and boolean operators; =vsum=, =vmean=, =vmin=, =vmax=, =vmedian=, =vcount=, common numeric functions; printf/Calc-style numeric formats; and =T=, =U=, and =t= duration modes. Later assignments override earlier assignments for the same cell. Empty-field =N= mode is supported. Formula lines, including unsupported forms, remain lossless.

Org2 deliberately does not execute Emacs Lisp formulas, remote-table references, or arbitrary Calc extensions. An unsupported or invalid formula produces a diagnostic and leaves the source unchanged rather than partially updating a table.

* Deliberate compatibility boundary

Org2 now covers most Org syntax used for notes, planning, publishing, linking, and scholarly prose. It deliberately does not implement macro expansion, Babel execution, inline source/call evaluation, or dynamic-block execution. Inline tasks remain ordinary deep headlines rather than a separate execution construct. Unsupported execution-oriented forms remain losslessly preserved as text or directives.
...
Canonical =.org= documents use an adjacent chart source block:
...
This is chart metadata over the preceding table. Chart source blocks may also point at a named table or materialized result elsewhere in the same note; fenced chart input remains accepted as typing sugar:
...
Line and bar charts may render multiple numeric columns against the same x-axis. List the columns in =y= (or =series= inside a chart source block), separated by commas:
...
Each line receives a stable color and grouped bar charts place each series beside the others within its x-axis category. Multi-series charts include a legend, series-aware accessibility labels, and series names in interactive tooltips. A =y-asc= or =y-desc= sort uses the first listed series. Histograms remain single-series because each row already represents one bucket.
...
Chart SVG uses CSS custom properties with opinionated defaults: =--org2-chart-mark=, =--org2-chart-series-2= through =--org2-chart-series-8=, =--org2-chart-surface=, =--org2-chart-axis=, =--org2-chart-grid=, =--org2-chart-label=, and =--org2-chart-title=. The first series retains =--org2-chart-mark= for compatibility. Set them on =.org2-chart= in a published stylesheet or the Mac app's =.org2/app.css= to customize the visual theme without changing the data declaration.

The CLI can render chart-affiliated tables directly:

#+begin_src sh
org2 render-chart --file report.org --block-id quarterly_revenue --format svg
#+end_src

* Local datasets and SQL views

Org2 can describe local/ad hoc data workflows with =dataset= and =sql= source blocks. Equivalent fences remain accepted as typing sugar. This is intended for notebook-like reports over explicit local files, read-only URLs, and named Org tables, with DuckDB as the first execution bridge:
...
#+begin_src sh
org2 query-data --file report.org --results fetches_by_state --out report.fetches.org
...
cat report.org | org2 query-data --stdin --results fetches_by_state --format json
#+end_src
...
#+begin_example
#+begin_src dataset package_fetches
type: clickhouse
...
#+begin_example
#+begin_src dataset recent_organizations
type: metabase
profile: scarf-metabase
query: |
...
#+begin_example
#+begin_src sql results=recent_organizations sources=recent_organizations_source freshness=24h
...
Use =type: table= with =source: NAME= when the source data already lives in a named Org table:
...
#+begin_src sh
org2 export beamer --file talk.org
org2 export beamer --file talk.org --out talk.tex --apply
org2 export beamer --file talk.org --out talk.pdf --pdf --apply
#+end_src
