...
This is ergonomic syntax sugar for chart metadata over the preceding table. The current renderer supports =bar=, =line=, and bucketed =histogram= charts over the previous table.
