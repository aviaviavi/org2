...
- Checkbox progress cookies: =[n/m]= and =[p%]= may appear in headings or list items. Compile output and lint compare each cookie with its innermost containing heading total; =org2 checkbox fix-cookies --file FILE --apply= writes the corrected values.
