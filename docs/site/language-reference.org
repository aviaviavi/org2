...
Org2 can describe local/ad hoc data workflows with fenced =dataset= and =sql= blocks. This is intended for notebook-like reports over explicit local files and named org2 tables, with DuckDB as the first execution bridge:
...
=org2 query-data= reads these blocks, creates DuckDB views for =csv=, =parquet=, =json=, or named org2 table datasets, runs a selected SQL result block through the local DuckDB CLI, and materializes the returned rows as a named org table or JSON envelope:
...
This is deliberately scoped to explicit local/ad hoc data sources. Org2 stores the source path or table name, query, and materialized result/provenance; secrets and remote warehouse credentials belong in external tooling, not in notes.
