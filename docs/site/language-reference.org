...
New =.org= documents use the ordinary Org form on disk. The Mac app rewrites fences to =#+begin_src= / =#+end_src= and rewrites backtick inline code to =~code~= whenever edited =.org= source is saved. This syntax canonicalization still occurs when general format-on-save is disabled: the input remains quick to type, while persisted =.org= files stay valid Org syntax. Existing =.org2= files and the default lossless parser/printer preserve their original spelling.

The CLI exposes the same explicit serialization profile:
...
=org2 table recalculate --file report.org2= previews the changed cells; add =--apply= to write them atomically. The Mac rendered view evaluates supported formulas for display and offers the same explicit recalculation action for editable files. HTML export uses the shared evaluator without rewriting source.
