...
Chart SVG uses CSS custom properties with opinionated defaults: =--org2-chart-mark=, =--org2-chart-background=, =--org2-chart-axis=, =--org2-chart-grid=, =--org2-chart-label=, and =--org2-chart-title=. Set them on =.org2-chart= in a published stylesheet or the Mac app's =.org2/app.css= to customize the visual theme without changing the data declaration.
