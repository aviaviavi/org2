...
Rendered clients treat an explicit =Color= or =Colour= table column as semantic presentation metadata. A whole-cell common color name or three/six-digit hex literal can be shown as a labeled swatch while the canonical cell remains ordinary text. The header opt-in is required: the same word in prose or another column has no color semantics. This is a rendering convention rather than new parser syntax, so source round-trips and non-visual clients retain the exact value.
