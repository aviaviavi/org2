...
The macOS Workspace exposes the same pipeline through *File → Export Slides as PDF…* and *File → Export Slides as LaTeX…*. See [[file:editors-macos.org][macOS Workspace → Slide export]] for the application workflow and troubleshooting.
