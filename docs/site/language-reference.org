...
Use =--stdin= when an editor, agent, or notebook-style client wants to run the current buffer without saving it first. Use =--line N= to select the SQL result block containing or after the cursor line. Relative file dataset paths are resolved from the current working directory for stdin input.
