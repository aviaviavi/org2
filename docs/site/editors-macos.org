...
The app is currently best understood as a local workspace shell for dogfooding the product direction: global capture, agenda review, approval review, file navigation, search, rendered block editing, meeting capture, OpenClaw handoff, knowledge-node creation, and crypt workflows over normal files.
...
The detail pane can render a selected file, agenda item, approval item, search result, backlink, meeting note, or agent thread. It supports both raw source editing and structured rendered-block editing.
