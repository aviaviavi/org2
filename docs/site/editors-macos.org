...
The iOS app can relay chat to a paired Mac over Tailscale. The Mac remains responsible for corpus access and model connections, so it must be awake with OpenOrg running. See [[file:getting-started.org::*iOS app][iOS setup]] for the current beta and pairing path.
