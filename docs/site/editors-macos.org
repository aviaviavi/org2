...
- *Runs & Review* :: use the Run Center to see active, completed, blocked, failed, and approval-needed durable agent runs; inspect their plan, cited inputs, artifacts, validation, handoffs, and comments; start, resume, retry, cancel, complete, approve/revise/reject, open the plain-text record, or save completed work as a reusable workflow. The Review Queue tab retains the parser-backed headline approval workflow and OpenClaw discussion path.
...
Typing =/= opens a discoverable command menu. Commands that only need compiler or app state run locally and leave their result in the chat transcript; commands marked *Agent* are expanded into a scoped request and sent with the selected document context. Unknown commands are not silently forwarded. Begin a message with =//= to send a literal leading slash.
