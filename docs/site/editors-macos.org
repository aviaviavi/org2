...
On first launch, a guided empty state offers *Open an existing corpus* or *Create a new corpus*. Opening accepts an existing folder of Org/Org2 files without migration. Creating asks for an empty folder, writes a minimal =org2.json= plus an inbox and welcome note, creates =notes/=, =daily/=, =views/=, and =compiled/=, then opens the welcome note. It refuses to initialize a non-empty folder so existing files are never silently overwritten.
...
The repository also ships the native =org2-lifecycle= plugin under =integrations/openclaw/=. It records substantial OpenClaw main-agent turns, subagent executions, and cron work in the same durable run model shown by Runs & Review. See [[file:getting-started.org][Getting started]] for the install path.
