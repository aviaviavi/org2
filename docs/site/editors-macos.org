...
If the gateway cannot be reached, for example when a network/VPN route is missing, the failed user message stays visible in the transcript with the transport error and a retry action. That failure state is persisted with the local transcript so a dropped send does not silently disappear.
