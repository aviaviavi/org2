...
Org2 Workspace is the product direction for focused clients over the same local corpus. The macOS app now exists as an alpha client for agenda review, file navigation, search, rendered block editing, meeting capture, OpenClaw handoff, agent-thread browsing, daily notes, autonomous loop artifacts, knowledge-node creation, and org-crypt workflows. Entity pages, richer meeting memory, capture loops, and data-backed reports should continue to use parser/compiler output instead of private app state.
...
- Workstreams surface for parallel slice kickoff, autonomous loop artifacts, knowledge-node creation, and first-run health checks,
- GPG-backed org-crypt configuration and encrypt/decrypt actions.

The next major app slice should lead with *autonomous capture and knowledge loops*: one-click creation of loop artifacts under =loops/=, meeting loops that retain transcript/source references, ID-backed knowledge-node creation, and status that remains visible in plain Org2 files. Meeting-to-action, knowledge-browser, and packaging/first-run work can now hang off the Workstreams surface in parallel without adding manual bottlenecks.
