...
- Portable personal/shared/project corpus identity, preview-first CLI initialization and validation, and remembered Mac mounts with explicit active-corpus switching
...
- identified personal/shared corpus mounts, explicit active-corpus switching, and a file scanner scoped to the active corpus,
...
The next collaboration milestone should add optional federated agenda/search with corpus identity on every result, followed by previewable private-to-shared handoff. Those views must preserve the explicit active destination for writes and must not silently combine agent access across mounted corpora. See [[file:collaboration.org][Shared corpora and collaboration]].
