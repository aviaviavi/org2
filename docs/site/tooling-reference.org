...
The transcript schema keeps the legacy =isArchived= field for older clients and adds =settledAt= plus =settlementSettings.autoSettleAfterSeconds=. Existing archived threads therefore load as settled, while older unclassified threads remain active. Auto-settlement requires non-empty inactive history and excludes the selected thread, pinned or unread threads, pending turns, and any failed, interrupted, or still-sending message.
...
An approval belongs to exactly one durable run and has an ID unique within that run. A run may have several pending approvals; it remains =waiting-approval= until every pending decision in the current approval boundary is recorded. Approving one item removes that exact =RUN_ID/APPROVAL_ID= pair from the queue but does not resume the run while another approval in that boundary is pending. Once all current decisions are in, all-approved runs return to =running=, while a rejected or revision-requested set moves to =blocked=. Requesting a replacement approval from that approval-derived blocked state opens a new boundary, so an approved revision resumes the run without rewriting the older rejected or revision-requested decision in its history. A separate clarification or operational block is not cleared by an approval decision. An approval may name both a required role and a specific reviewer. =approval-decide= refuses a mismatched =--role= or =--actor=, and the decision event preserves the asserted role plus any release receipt. This is a durable workflow boundary; operating-system or organizational identity enforcement can sit above it.

=org2 approvals= returns the unified =org2:approvals:v2= decision queue. Its =kind= field distinguishes durable =run= approvals from standalone =headline= approvals; run items include =runId=, =approvalId=, the run goal/status, and pending/total approval counts. Deciding a run item must use =run approval-decide= so the queue and Agent Work's *Runs* tab are two projections of the same canonical =.org2/runs/*.org2= record. Standalone heading approvals remain ordinary corpus TODOs and keep their file/line/ID identity. =org2 review list= returns the broader =org2:review-queue:v1= inspection feed, which also includes review-required artifacts, blocked clarification requests, and warning/failed validation.
