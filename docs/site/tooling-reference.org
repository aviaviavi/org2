...
OpenOrg can eventually bundle its registered desktop client credential pair through the =OpenOrgGoogleOAuthClientID= and =OpenOrgGoogleOAuthClientSecret= application properties. Until then, download the OAuth client JSON for a Google Cloud client of application type *Desktop app* and choose *Import Client JSON…* in the publish sheet; the client ID and client secret can also be pasted separately. OpenOrg masks the imported client secret and stores it with the OAuth credential in macOS Keychain. It sends the pair only to Google's token endpoint for authorization-code exchange and refresh; it never passes the client secret to Org2, the corpus, a published artifact, or a command-line argument. Google's installed-app guidance notes that some clients receive a client secret but that installed applications cannot keep one confidential in the same sense as a server application. The flow still uses a loopback redirect, PKCE, and a per-request state value. Developers may provide =ORG2_GOOGLE_OAUTH_CLIENT_ID= and =ORG2_GOOGLE_OAUTH_CLIENT_SECRET= when building a configured app. See [[https://developers.google.com/identity/protocols/oauth2/native-app][OAuth 2.0 for desktop apps]], [[https://developers.google.com/identity/protocols/oauth2][Google OAuth 2.0 overview]], and [[https://developers.google.com/workspace/drive/api/guides/api-specific-auth][Drive authorization scopes]].

Publishing to Google is deliberately one-way. Save the returned file ID and Drive version in the calling app's private destination state. Replacing an existing Doc, Slides deck, Sheet, or PDF requires all three guards: =--document-id ID=, =--if-version VERSION=, and =--replace-existing=. Org2 fetches the current Drive record first and refuses the upload when the version changed, so a collaborator's edits are not silently overwritten. It also refuses full-content replacement when the target has comments, because replacement could detach their anchors even when comments did not advance the file version. The caller can then import remote changes or publish a new copy. Permissions, comments, suggestions, and Google revision history belong to Google; Org2 does not attempt bidirectional synchronization.
