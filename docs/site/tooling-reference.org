...
Run =npm run org2 -- --help= for current top-level usage. Proposed AI job commands are documented separately in [[file:ai-processing-architecture.org][AI processing architecture]] so the current deterministic CLI surface stays clear.
