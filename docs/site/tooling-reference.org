...
=org2 ledger= keeps high-volume recurring bookkeeping out of a monolithic agent prompt or TODO file. One stable account becomes one canonical Org2 file. Account Outreach uses =notes/account-outreach/accounts/ACCOUNT_ID.org2=; the same command can use another ledger name for another account-oriented process.
