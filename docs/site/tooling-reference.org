...
Run =npm run org2 -- --help= for current top-level usage. AI architecture and job design are documented separately in [[file:ai-processing-architecture.org][AI processing architecture]] and [[file:ai-job-manifests.org][AI job manifests]] so the deterministic CLI surface stays clear.
