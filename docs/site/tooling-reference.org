...
#+begin_src json
{
  ...
  "contributes": {
    ...
    "templates": [
      { "id": "card-note", "path": "templates/card.org2" }
    ]
  }
}
#+end_src
...
Deterministic formatting for Org and Org2 files. Add =--canonical-org= to rewrite accepted input conveniences such as fenced source blocks, =#+begin_org2= aliases, and backtick inline code to ordinary Org syntax. OpenOrg uses this profile whenever edited =.org= source contains those conveniences; =.org2= files retain lossless spelling. Syntax canonicalization is independent of the app's optional general format-on-save setting.

#+begin_src sh
org2 fmt --dir ~/notes --recursive --check
org2 fmt --dir ~/notes --recursive --apply
org2 fmt --file note.org --canonical-org --apply
#+end_src
...
org2 publish document --file reports/brief.org2 --to web --out-dir publish/brief
org2 publish document --file reports/brief.org2 --to web --out-dir publish/brief --apply
org2 publish document --file talks/briefing.org2 --to beamer-pdf --out-file publish/briefing.pdf
org2 publish document --file reports/brief.org2 --to google-docs --folder-id DRIVE_FOLDER_ID
org2 publish document --file talks/briefing.org2 --to google-slides --folder-id DRIVE_FOLDER_ID
org2 publish document --file reports/evidence.org2 --to google-sheets --folder-id DRIVE_FOLDER_ID
...
ORG2_GOOGLE_DRIVE_ACCESS_TOKEN="$TOKEN" \
  org2 publish document --file reports/brief.org2 --to google-docs \
  --folder-id DRIVE_FOLDER_ID --apply --format json

# Other native Google formats use the same credential and folder policy.
ORG2_GOOGLE_DRIVE_ACCESS_TOKEN="$TOKEN" \
  org2 publish document --file talks/briefing.org2 --to google-slides \
  --folder-id DRIVE_FOLDER_ID --apply --format json

ORG2_GOOGLE_DRIVE_ACCESS_TOKEN="$TOKEN" \
  org2 publish document --file reports/evidence.org2 --to google-sheets \
  --folder-id DRIVE_FOLDER_ID --apply --format json
