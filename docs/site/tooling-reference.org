...
org2 run complete RUN_ID --dir ~/notes
#+end_src

Other lifecycle operations include =resume=, =retry=, =block=, =fail=, =cancel=, =fork=, =assign=, =comment=, and =artifact=. =run normalize= converts legacy =AGENT_RUN_ID= headings into shared records while retaining a citation to their source location.
