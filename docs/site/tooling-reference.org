...
org2 todo set --file notes.org --line 42 --keyword SKIPPED --apply
org2 plan set --file notes.org --line 42 --kind scheduled --date 2026-03-25 --apply
#+end_src

Set corpus defaults through Settings → Workspace → TODO States in OpenOrg, or with the guarded CLI:

#+begin_src sh
org2 todo-config show --dir ~/notes
org2 todo-config set --dir ~/notes --sequences-json '["TODO WAITING | DONE SKIPPED"]'
org2 todo-config set --dir ~/notes --sequences-json '["TODO WAITING | DONE SKIPPED"]' --apply
#+end_src
...
#+begin_src sh
org2 checkbox --file notes.org --line 12
org2 checkbox --file notes.org --line 12 --apply
org2 checkbox set --file notes.org --line 12 --status checked --apply
org2 checkbox --file notes.org --line 12 --format json
#+end_src
...
Writes require =--apply=. The default output is a diff; =--format text= returns the complete edited source, and =--format json= returns an =org2:checkbox-edit:v1= envelope with the original =revision=, old/new states, and =changed=/=applied= flags. Pass the preview's =revision= as =--if-revision= when applying later to reject stale edits. =column= is the zero-based UTF-16 position of the marker character, suitable for LSP clients.

Only the selected marker changes. Indentation, list numbering, counter cookies, other text, line endings, final-newline presence, and file permissions are preserved. Parent progress cookies are not recalculated. Setting an already-selected state reports no change.
...
=scope.kind= is =file=, =heading= or =all=; optional =filePrefix= is a case-sensitive corpus-relative path prefix. =columns= supports 1–24 fields. Lowercase built-in fields =title/file/kind/todo/tags/id= are read-only; other field names normalize to uppercase property keys. =match= combines zero to 30 filters with =all= or =any=; zero filters select every scoped row. Operators =is/isNot/contains= compare case-insensitive strings, =exists/missing= test nonempty/empty values, and =gt/lt= compare finite numeric values. Missing values never satisfy numeric comparisons. =sort= supports up to eight fields with =asc/desc=; two numeric values sort numerically, otherwise case-insensitive lexical order applies, with source keys as stable tie-breakers. Optional =groupBy= emits a group value per row; clients preserve sorted order within each group. =limit= is 1–5,000; =total= and =truncated= report omitted rows.

#+begin_src sh
org2 property-view list --dir CORPUS
org2 property-view query --dir CORPUS --view open-work
org2 property-view query --dir CORPUS --definition '{"schema":"org2:property-view:v1","id":"all-notes","title":"All notes","layout":"table","scope":{"kind":"file"},"columns":["title","ASSIGNEE"],"match":"all","filters":[],"sort":[],"limit":500}'
org2 property-view save --dir CORPUS --definition 'JSON_DEFINITION'
org2 property-view save --dir CORPUS --definition 'JSON_DEFINITION' --apply
org2 property-view edit --dir CORPUS --file notes/work.org --kind heading --line 12 --property STATUS --value review --if-revision 'sha256:SOURCE_REVISION'
#+end_src
...
The =canvas= family operates on [[https://jsoncanvas.org/spec/1.0/][JSON Canvas 1.0]] documents. The =.canvas= file is canonical; native board layout and local resource previews consume shared TypeScript output.

#+begin_src sh
org2 canvas create --dir ~/notes --file boards/project.canvas
org2 canvas create --dir ~/notes --file boards/project.canvas --apply
org2 canvas show --dir ~/notes --file boards/project.canvas --json
org2 canvas targets --dir ~/notes --query architecture --json
org2 canvas edit --dir ~/notes --file boards/project.canvas --if-revision sha256:HASH --stdin --apply --json
org2 canvas import --dir ~/notes --file boards/imported.canvas --from /path/to/source.canvas --apply
org2 canvas export --dir ~/notes --file boards/project.canvas --out /path/to/new-copy.canvas --apply
#+end_src
...
#+begin_src sh
org2 roam connections --dir ~/notes --id alpha-stable --depth 2 --format json
org2 roam connections --dir ~/notes --file ~/notes/project.org --line 17 --format json
org2 roam mention-link --dir ~/notes --file ~/notes/meeting.org --mention KEY --target alpha-stable --if-revision sha256:HASH --format json
...
=mention-link= requires one occurrence key, an explicit destination ID, and the source's =--if-revision=. It rechecks the occurrence and current target index, refuses duplicate target IDs and stale source, and previews the resulting source. =--apply= makes one atomic revision-guarded write preserving line endings and file mode. It never batch-links ambiguous text or publishes a corpus. OpenOrg's Context → Graph panel consumes these same commands.
...
=org2 server init|start|status|pair|revoke|stop|permissions|assign|service|push-config= manages the macOS headless relay and scheduler. Configuration is machine-local; =server permissions --filesystem-access read-only|workspace-write|full-access= previews or atomically updates that host's local Codex and Claude Code filesystem policy, with a restart required after a change. =server status= reports the active policy. =server assign= previews a symbolic =automationHostRef= change in =org2.json=. The =workflow due --host-ref HOST= and scheduled =workflow run --host-ref HOST= interfaces enforce that owner. See [[file:headless-server.org][Headless server setup]] for the complete lifecycle and limitations.

* Project note commands

=org2 project list|show|create|adopt|update= operates on ordinary Org files.
List returns =org2:project-list:v1= with projects and per-file diagnostics;
create, adopt, and update return =org2:project-edit:v1= with proposed content,
project metadata, =baseRevision= (null for new files), and =applied=.
Mutations preview unless =--apply= is supplied.

Create accepts =--title=, =--color=, =--file=, and =--id=. Preserve the preview's
ID and path when applying. The default path is beneath =roam.nodesDir= (or
=notes=), in =projects/=. Optional =--description TEXT= supplies the initial brief;
without it, creation adds no placeholder sections or TODOs. Adopt takes an existing file path and preserves its
body and file-level ID. Pass an adoption preview's =--id= and =--if-revision=
using its project ID and =baseRevision= when applying. Update accepts =--thread UUID=, =--remove=, and
=--color=. Adopt/update accept =--if-revision sha256:...= for guarded writes.

Color is optional; omitted =PROJECT_COLOR= means no color. Use =--color none=
to clear a color. Colors may be blue, teal, green, orange, red, purple, gray, or a six-digit
hexadecimal RGB value such as =#4A90D9=. Project
notes may live anywhere in nonhidden corpus directories, respecting configured
ignore patterns. Put the marker in the first 8 KiB; notes are limited to 1 MiB.
Duplicate IDs and malformed notes produce diagnostics instead of ambiguous
membership. Briefs contain at most 12,000 characters. =workspace agent-state=
also exposes an independently fallible =projects= section.

The metadata is =#+ORG2_KIND: project=, =#+TITLE:=, =#+PROJECT_COLOR:=,
=#+PROJECT_THREADS:= (space-separated chat UUIDs), and a file-level properties
drawer containing =:ID:=. All remaining content uses normal Org headings,
TODOs, tags, planning, and links. There is no separate project database.
...
#+begin_src sh
org2 browser-clip import --file article.org2clip --dir ~/notes --json
# Copy revision and clipRevision from that preview:
org2 browser-clip import --file article.org2clip --dir ~/notes \
