...
=org2 render-chart= renders an SVG artifact from =#+chart:= or =#+plot:= metadata attached to an org2 table. This gives editor clients a shared backend instead of making each extension reimplement table parsing and chart semantics.
...
The first renderer supports deterministic =bar= and =line= charts with =x= and =y= column mappings, for example =#+chart: line x=quarter y=revenue=. JSON output includes =ok=, =format=, =artifact=, =source=, =diagnostics=, and =svg= so VS Code, Vim, or agent tooling can show useful errors for missing columns, unsupported chart types, or missing chart blocks.
