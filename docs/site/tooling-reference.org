...
In OpenOrg for macOS, choose *Publish Document… → Local Link* to run this projection and host a *Web page*, document *PDF*, or compiled *Beamer slides (PDF)* directly from the Mac behind a high-entropy capability URL. The first local host is session-bound, available only while OpenOrg is running, and uses HTTP on the local/private network; do not use it on an untrusted network. The result screen can copy or open the link and revoke it immediately. *Settings → Sharing* is the persistent management surface for this app session: it lists the source path, output format, URL, and start time for every active link and can open, copy, stop, or stop all publications.
