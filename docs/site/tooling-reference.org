...
org2 checkbox fix-cookies --file notes.org
org2 checkbox fix-cookies --file notes.org --apply
#+end_src
...
Use =fix-cookies= to recalculate stale =[n/m]= and =[p%]= progress cookies across the file. Each cookie uses the checkbox total from its innermost containing heading; content before the first heading uses the file preamble. This prevents a recursive ancestor total from overwriting a correct descendant cookie. The ordinary =cycle= and =set= actions still change only the selected checkbox marker and do not recalculate cookies implicitly.
...
org2 run create --title "Resolve the customer issue" --goal "Investigate and resolve the customer issue" \
...
#+begin_src sh
org2 run create --title "Prepare the weekly operating review" \
