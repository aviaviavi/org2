...
org2 run approval-request RUN_ID --title "Release report" \
  --action "publish operating review" --risk external-action --role owner \
  --material-file views/release-approval.json --dir ~/notes
org2 approvals --dir ~/notes --recursive --format json
org2 approval decide run:RUN_ID:APPROVAL_ID --decision approved \
  --actor avi --role owner --expected-fingerprint sha256:... --dir ~/notes --apply
...
Other lifecycle operations include =resume=, =retry=, =block=, =fail=, =cancel=, =complete-external=, =fork=, =assign=, =comment=, =outcome=, =runtime=, =artifact=, and =artifact-review=. =run runtime= records observable provider/model identity and optional token, cost, or elapsed usage without storing credentials. =run artifact-review= records a human review decision on an existing artifact and, for a linked Org/Org2 file inside the corpus, updates its =ORG2_REVIEW_STATUS= metadata at the same time; it also repairs stale review state on historical completed records. Normal completion requires =--summary= with a human-readable result and refuses pending approvals, unsatisfied declared approval requirements, or =review-required= artifacts; repeat =--highlight= and =--next-action= to populate the reviewer-facing outcome. When a person confirms that a blocked outcome was completed outside this workflow, =run complete-external RUN_ID --summary "Where or how it was completed" --actor NAME= records an explicit =completed-externally= audit event, marks unfinished workflow steps skipped, and closes the run while preserving unresolved legacy approvals, artifact review states, and validations as history. It does not bypass declared approval requirements; a guarded step must already be approved or explicitly skipped because no effect occurred. Agents must not infer external completion on their own. =run outcome= may record or revise an outcome before completion. Blocking requires =--reason= with the specific clarification or next action; Org2 rejects an unactionable reasonless blocked state. =run normalize= converts legacy =AGENT_RUN_ID= headings into shared records while retaining a citation to their source location.

An approval belongs to exactly one durable run and has an ID unique within that run. New requests also carry a deterministic =sha256:= fingerprint over the title, action, risk, reviewer assignment, request note, and structured exact material. The ID names the immutable request version; the fingerprint binds what the reviewer saw. Decision notes no longer overwrite request notes. If material changes, request a new approval with =--supersedes OLD_ID=; the old pending request becomes =revised= while remaining in history. Provider or runtime IDs are correlation data, not an alternate approval authority. Before a protected external effect, =run approval-effect-reserve= atomically binds the approved fingerprint and material digest to one tool-call ID. A matching =run approval-effect= consumes that reservation and records the provider receipt. Ambiguous outcomes remain reserved and fail closed; only explicit human/provider reconciliation may clear them with =run approval-effect-release=, an actor, and a reason.

A run may have several pending approvals; it remains =waiting-approval= until every pending decision in the current approval boundary is recorded. Approving one item removes that exact =RUN_ID/APPROVAL_ID= pair from the queue but does not resume the run while another approval in that boundary is pending. Once all current decisions are in, all-approved runs return to =running=, while a rejected or revision-requested set moves to =blocked=. A superseding request replaces the older request inside that boundary without making the older revision count as a rejection of the replacement. A separate clarification or operational block is not cleared by an approval decision. An approval may name both a required role and a specific reviewer. The decision command refuses a stale =--expected-fingerprint=, mismatched =--role=, or mismatched =--actor=. This is a durable workflow boundary; operating-system or organizational identity enforcement can sit above it.

=org2 approvals= returns the additive =org2:approvals:v2= decision queue. Its =kind= field distinguishes durable =run= approvals from standalone =headline= approvals; every item includes =queueId=, =fingerprint=, =binding=, =canApprove=, and an optional reason that approval is blocked. Run items also include =runId=, =approvalId=, exact structured material when present, the run goal/status, and pending/total approval counts. A request bound to a declared workflow approval also includes a nested =requirement= projection with its stable ID, descriptor, optional guarded step, and current state. Submit both kinds through =org2 approval decide QUEUE_ID --expected-fingerprint ...=. The command dispatches native items to their canonical =.org2/runs/*.org2= record and compare-and-swaps legacy headline items at the currently cited file and heading identity. Identity-less legacy headings use a provisional line locator, so refresh the queue after moving one. Decisions for either binding preview unless =--apply= is passed. Existing =run approval-decide= and =todo approve= commands remain compatibility surfaces over the same locked decision semantics. =org2 review list= returns the broader =org2:review-queue:v1= inspection feed, which also includes review-required artifacts, blocked clarification requests, and warning/failed validation.

* Reusable workflows and triggers

=workflow save RUN_ID= converts a completed run into a draft =org2:workflow:v1= file under the visible top-level =workflows/=. Packages declare semantic version, compatibility, parameters and defaults, context rules, deterministic/agent/tool/approval steps, capabilities, risk, outputs, validation, approval boundaries, and manual/schedule/file-change/capture/meeting-import triggers. Each approval declaration has a stable ID and may name a =beforeStepId=. Instantiation copies those declarations into the run as immutable approval requirements; older declarations without IDs receive deterministic =workflow-approval-N= IDs. Use =run approval-request RUN_ID --requirement REQUIREMENT_ID --material-file ...= to bind exact review material. Starting or completing a guarded step and completing the run both fail closed until the latest request for every applicable requirement is approved. A fork keeps the declarations but intentionally resets their approval history. See [[file:workflows.org][Workflows]] for the plain-text ownership and OpenClaw execution contract.
...
Schedule syntax is intentionally deterministic and small: =every 15m=, =every 4h=, or =every 1d=. Event hosts ask =workflow triggers= which declarations are due, then instantiate the workflow; =--event= accepts only =capture= or =meeting-import= and rejects unknown event names. The durable run remains the execution record.
