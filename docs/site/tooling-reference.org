...
=org2 approvals= returns the unified =org2:approvals:v2= decision queue. Its =kind= field distinguishes durable =run= approvals from standalone =headline= approvals; run items include =runId=, =approvalId=, the run goal/status, and pending/total approval counts. Deciding a run item must use =run approval-decide= so the queue and Run Center are two projections of the same canonical =.org2/runs/*.org2= record. Standalone heading approvals remain ordinary corpus TODOs and keep their file/line/ID identity. =org2 review list= returns the broader =org2:review-queue:v1= inspection feed, which also includes review-required artifacts, blocked clarification requests, and warning/failed validation.
