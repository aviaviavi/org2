...
Use JSONL for larger corpora when downstream tools prefer one record per line. If you write compiled artifacts into a conventional =compiled/= zone, add normal =ORG2_ARTIFACT_*= metadata/provenance to any Org wrapper files and keep using =org2 lint= for generated-output health checks.
