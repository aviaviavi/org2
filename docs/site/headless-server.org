...
Local Codex and Claude Code permissions belong to the execution host. The desktop setting applies when iOS is paired directly with that desktop; an iPhone connected to a headless server uses the server's machine-local =localAgentFilesystemAccess= setting. New configurations default to =workspace-write=. Choose a different mode during initialization, or update an existing stopped server with a preview and then restart it:
...
The accepted values are =read-only=, =workspace-write=, and =full-access=. Full access lets local Codex and Claude Code write anywhere the server's macOS account can and does not provide interactive approval prompts, so use it only on a trusted host for trusted conversations. Applying =server permissions= changes the private configuration atomically and reports whether a restart is required; it does not change a running turn. =server status= reports the active worker's =filesystemAccess= value so the effective host policy can be verified after restart.

The default destination is local Codex using its existing login. =--destination claude= or =--destination openclaw= selects the corresponding built-in adapter. Additional destination configurations use the same fields as the desktop. OpenClaw's built-in destination honors its configured =endpoint= and =agentID=; empty values use the host's gateway settings and =main= agent. Provider credentials stay in runtime login stores, protected gateway configuration, or Keychain. The headless worker never waits for interactive Keychain dialogs: inaccessible credentials fail the affected operation instead of blocking the relay. If access is denied, unlock or authorize the existing credential in a logged-in session on the host. Setup does not copy data or credentials from another machine.
