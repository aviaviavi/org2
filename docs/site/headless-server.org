...
The desktop continues to operate its local corpus and chats. This version does not make the desktop a client of the server relay or coordinate simultaneous transcript writers across replicated corpora. When hosting an existing corpus, continue its chats through the server and avoid simultaneous desktop chat editing against another synced copy. This also avoids diverging copies of agent runtime sessions. Agents on the server can use =org2 thread post= to deliver idempotent background results to the same conversation.
