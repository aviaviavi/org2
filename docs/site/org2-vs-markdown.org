...
| Editor independence | High for prose, weaker for app semantics | CLI/LSP/compiler model designed for many editors |
...
That makes Org2 a better fit for long-lived research notes, project memory, meeting trails, and roam-style workflows where a heading can be a real addressable object, not just text inside a file.

** You want structure without leaving plain text

Markdown can grow extensions forever: front matter, task-list syntax, wiki links, callouts, custom attributes, app metadata, and fenced conventions. The problem is not that these are bad. The problem is that every app chooses a different subset.
...
The payoff is that tools do not have to guess what the text means.
...
That makes Org2's explicitness less like clerical overhead and more like a target format for collaboration between humans, editors, scripts, and agents.
...
- editor features through the shared LSP backend

That is the core pitch: Org2 is not only a markup language. It is a compiler/runtime for personal and team knowledge.
...
That matters for automation. A script or agent should not have to scrape a rendered page or reverse-engineer an editor extension. It should be able to ask the corpus for tasks, links, sources, provenance, generated artifacts, and publishable output.
...
Org2 gives those workflows natural boundaries: raw source material, reviewed notes, compiled semantics, derived views, generated drafts, and published output. Properties such as artifact role and provenance let generated work stay inspectable instead of quietly blending into durable knowledge.
