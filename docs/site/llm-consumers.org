...
=org2 agent fetch --id thread-1 --format json= exposes this as =thread.contextAttachments= along with =thread.agent=, =thread.session=, =thread.status=, =thread.transcript=, and =thread.storage=. Attachments that point at known =id:=, =file:=, or wiki/note targets include a =target= object with the resolved title, file, line range, and citation. This gives Workspace clients a stable way to reopen a chat from a selected TODO/report/entity while preserving citations and avoiding raw transcript storage unless the user chooses it.
