...
New human-facing documents use =.org=. In =.org= files, formatting and save-time shorthand handling rewrite fenced blocks and backtick inline code to ordinary Org syntax. Existing =.org2= files retain their lossless spelling.
