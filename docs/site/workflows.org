...
Automation threads use the configured destination model, or the runtime default when none is configured. They do not inherit model or reasoning overrides from interactive chats. Ordinary new chats continue to remember those choices. This keeps unrelated chat preferences from forcing an OpenClaw automation to require the live Gateway when HTTP compatibility would otherwise be available; it does not bypass authentication failures or explicit destination model requirements.
