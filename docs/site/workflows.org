...
2. Records the schedule, planned time, destination reference, agent profile, and stable logical-work ID, then carries the file-owned model and reasoning effort into dispatch.
...
Automation threads use the workflow's exact =model= and =reasoningEffort= when present. Otherwise they use the configured destination model and runtime reasoning default. They do not inherit overrides from interactive chats. Ordinary new chats continue to remember those choices. A runtime that rejects a pinned value fails that attempt visibly; OpenOrg does not retry a file-configured automation with different defaults.
