...
High-volume account-oriented workflows should keep global selection state in =org2 ledger= accounts rather than in the workflow prompt or one append-only task heading. A workflow can query =ledger list LEDGER --eligible --json=, link the exact run approval through an idempotent account event, and record the provider receipt after the approved action completes. The workflow remains the recipe, each run remains one attempt, and the ledger remains durable cross-attempt subject history.
...
5. Pausing the workflow disables its managed OpenClaw job.
