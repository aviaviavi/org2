...
A workflow declares capabilities and a risk class. Expected outputs should land in reviewable locations such as =views/= or =compiled/=, and consequential or external actions should carry an explicit approval requirement. Each declaration may have a stable =id= and a =beforeStepId=; older declarations receive a deterministic ID when instantiated. The run copies those declarations as immutable intent, while concrete approval request versions carry the changing exact material. Use =org2 run approval-request RUN_ID --requirement REQUIREMENT_ID --material-file ...= to bind the two. Org2 refuses to start or complete a guarded step, or complete the run, until the latest request is approved. Skipping a guarded step is allowed only with an explicit detail recording why no protected effect occurred, and forks retain the requirements while resetting all approval history.

An external-action or high-impact approval must bind the exact recipient, content, command, and attachments as typed approval material; an opaque identifier, a vague note, or a fingerprint by itself is not enough to approve. The Mac Agent Work surface retrieves correlated runtime details when they remain available and disables *Approve* when their bound material cannot be shown and verified. Activating a schedule never weakens those boundaries.
