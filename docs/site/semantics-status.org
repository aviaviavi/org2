...
Org2 treats linked knowledge as compiler state, not just editor decoration. A note corpus should be able to answer three practical questions reliably:
...
That separation matters because users need dry-run reports and safe, reviewable edits. A linkify command, for example, should be able to explain why it suggested a link before it changes a file.

** High-leverage next slices

- Make cited query/search output available as a stable CLI surface for knowledge-base workflows.
- Expand roam linkify reports so suggestions include represented-node/alias reasoning.
