...
| Source files | Local Markdown, properties, Bases files, and open JSON Canvas files | Local =.org= and =.org2= files, JSON Canvas files, portable property-view JSON definitions, and explicit generated artifacts |
...
| Linked knowledge | Backlinks, outgoing links, graph, local graph, Canvas | IDs, wiki/file links, backlinks, live note/heading embeds, interactive local graphs, explicit unlinked-mention linking, JSON Canvas, cited queries, and link-health operations |
| Structured views | Bases tables, lists, cards, maps, formulas, and community views | Saved editable property tables/cards with filtering, sorting and grouping; agenda/query outputs, data notebooks, materialized SQL views and charts |
...
Org2 offers saved table and card views over note and heading properties, with a no-SQL filter/sort/group builder and revision-guarded source editing in the Mac app. Portable view definitions live in corpus-relative JSON files. Data notebooks also cover explicit datasets, SQL materialization, freshness, and report evidence. OpenOrg also edits JSON Canvas boards for notes, images, text, groups, and connections, with revision-guarded saves and portable import/export.
