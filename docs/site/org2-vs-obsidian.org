...
| Maturity | Established desktop and mobile products with a large community | Early alpha with working CLI, Mac app, VS Code integration, and source-distributed iOS app |
...
The Mac *Runs & Review* surface presents that state as a workflow for coordinating with agents. OpenClaw is the current end-to-end integration; the underlying records and interfaces are provider-independent.
...
Org2 is early alpha. Its Mac app is useful but still evolving, the iOS app is distributed as source, VS Code has the strongest general editor integration, and the plugin/theme ecosystem is small. Packaging, capture/refile depth, visual graph exploration, and cross-platform polish remain behind Obsidian.
