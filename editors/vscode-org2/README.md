...
- `org2.editor.refreshAfterCliApply`: when true (default), CLI write commands (TODO/planning/archive plus Roam ID/backlink writes) force a targeted file refresh from disk (`todo`/`plan`/`roam` ID ensures only refresh when CLI output reports an actual change); set false to rely on VS Code file watching and avoid refresh-related fold churn.
