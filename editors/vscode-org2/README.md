...
- `org2.agenda.timeFilter`: filter by planning clock-time tokens (exact `HH:MM` values plus `timed`/`untimed`; comma-separated terms use OR matching)
...
- `org2.agenda.excludeTimeFilter`: exclude rows by planning clock-time tokens (exact `HH:MM` values plus `timed`/`untimed`; comma-separated terms use OR matching)
