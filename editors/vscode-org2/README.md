...
- `org2.agenda.sortBy`: optional per-day sort order (`default`, or comma-separated keys like `file,headline,todo,priority,effort,level,kind,tags,line`); prefix a key with `-` (or suffix with `:desc`) for descending order
