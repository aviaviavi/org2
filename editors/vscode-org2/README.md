...
- `org2.agenda.fileOrder`: optional custom file-path fragment ordering used by `sortBy=file` / `groupBy=file` (comma-separated case-insensitive terms like `projects,work,inbox`; matching files rank first in provided order, unmatched paths keep lexical fallback)
