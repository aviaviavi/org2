...
- `org2.agenda.sortBy`: optional per-day sort order (`default`, or comma-separated keys like `file,headline,todo,priority,effort,level,time,kind,tags,line`); prefix a key with `-` (or suffix with `:desc`) for descending order
- `org2.agenda.groupBy`: optional per-day grouping keys (`default`, or comma-separated keys like `todo,file`); uses the same key syntax as `sortBy` (including `time` and `-key`/`:desc`)
