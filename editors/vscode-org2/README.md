...
- `org2.editor.refreshNonActiveOpenFiles`: when false (default), post-apply refresh only touches the active target editor. Set true to also refresh non-active files, but only when they are open and currently visible in an editor group.
