...
- `org2.agenda.statusOrder`: optional custom normalized status-bucket ordering used by `sortBy=status` / `groupBy=status` (comma-separated values from `todo,in_progress,done,canceled,custom`; aliases like `all`, `active`, `actionable`, `open`, `prog`, `completed`, and `closed` are accepted)
