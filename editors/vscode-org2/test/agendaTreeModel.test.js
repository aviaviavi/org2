...
test('buildAgendaTreeGroupsFromCli honors priority:desc sort syntax', () => {
  const groups = buildAgendaTreeGroupsFromCli(
    {
      overdue: [],
      days: [
  ...
  assert.deepEqual(
    groups[0].items.map((item) => item.headline),
    ['none item', 'c item', 'b item', 'a item']
  );
});

test('buildAgendaTreeGroupsFromCli supports prio alias for local priority sorting', () => {
  const groups = buildAgendaTreeGroupsFromCli(
    {
      overdue: [],
      days: [
