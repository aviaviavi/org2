function resolveAgendaDays(filter, defaultDays) {
  if (filter && filter.type === 'today') return 1;
  if (filter && filter.type === 'next') return filter.days;
  return defaultDays;
}
...
function readAgendaCliOptions(cfg, agendaRoot, filter, resolveAgendaFiles) {
  ...
  const days = resolveAgendaDays(filter, defaultDays);
