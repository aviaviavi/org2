...
private struct HomeView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      HeaderBar(title: "Home", subtitle: homeSubtitle, surface: .home) {
        Button {
  ...
  private var homeSubtitle: String {
    store.corpusRoot?.path ?? "Open an org corpus to begin"
  }
}
...
private struct HomeSummaryGrid: View {
  ...
  var body: some View {
    HStack(spacing: 8) {
      StatTile(label: "Files", value: "\(store.corpusFiles.count)")
      StatTile(label: "Today", value: "\(store.agenda?.todayItemCount ?? 0)")
      StatTile(label: "Meetings", value: "\(store.meetings.count)")
    }
  }
}
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 8)], alignment: .leading, spacing: 8) {
        HomeActionButton(title: "Agenda", systemImage: "calendar") {
          store.selectedSurface = .agenda
        }
        HomeActionButton(title: "Files", systemImage: "doc.text") {
          store.selectedSurface = .files
        }
        HomeActionButton(title: "Search", systemImage: "magnifyingglass") {
          store.selectedSurface = .search
          store.searchFocusToken &+= 1
        }
        HomeActionButton(title: "Meetings", systemImage: "mic") {
          store.selectedSurface = .meetings
        }
        HomeActionButton(title: "OpenClaw", systemImage: "sparkles") {
...
private struct HomeActionButton: View {
  ...
  var body: some View {
    Button(action: action) {
      ...
      .background(WorkspaceDesign.subtleFill, in: RoundedRectangle(cornerRadius: 7, style: .continuous))
      .overlay(
...
private struct HomeRecentFiles: View {
  @EnvironmentObject private var store: WorkspaceStore

  private var recentFiles: [CorpusFile] {
    Array(store.corpusFiles.sorted { lhs, rhs in
