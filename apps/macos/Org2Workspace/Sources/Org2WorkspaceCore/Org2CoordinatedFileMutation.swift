...
enum Org2CoordinatedFileMutation {
  ...
  static func mutateTextAtomically<Result>(
    ...
    expectedText: String? = nil,
    createIfMissing: Bool = false,
    _ mutation: (String, Bool) throws -> (text: String, result: Result)
  ) throws -> Result {
    try withMutationLock(for: fileURL) {
      ...
      let mutationResult = try mutation(current, existed)
  ...
  static func writeTextAtomicallyIfUnchanged(
    _ text: String,
    to fileURL: URL,
    expectedText: String
  ) throws {
    try mutateTextAtomically(at: fileURL, expectedText: expectedText) { _ in
      (text, ())
    }
  }
  ...
  private static func replaceAtomically(fileURL: URL, text: String) throws {
    ...
    let temporaryURL = fileURL
      .deletingLastPathComponent()
      .appendingPathComponent(".\(fileURL.lastPathComponent).\(ProcessInfo.processInfo.processIdentifier).\(UUID().uuidString).tmp")
    let descriptor = Darwin.open(
      temporaryURL.path,
      O_WRONLY | O_CREAT | O_EXCL,
      mode_t(permissions)
