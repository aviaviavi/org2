...
final class MobileRemoteHTTPServerTests: XCTestCase {
  func testParsesAuthenticatedJSONRequestAfterFullBodyArrives() throws {
    let body = try MobileRemoteProtocol.encoder().encode(
      MobileRemoteSendMessageRequest(content: "Continue the review")
    )
    let header =
    ...
    XCTAssertEqual(try request.decode(MobileRemoteSendMessageRequest.self).content, "Continue the review")
  }
