...
* SOURCE blocks (#+begin_src / #+end_src / fenced ``` blocks)
...
Backtick code spans (=`...`=) follow the same boundary rules as other inline emphasis markers in v0.
