...
export async function runPropertyViewsCommand(args: string[]): Promise<void> {
  ...
  if (!["list", "query", "suggest", "save", "edit"].includes(action ?? "")) throw new Error("Expected property-view list, query, suggest, save or edit");
  const flags = new Map<string, string>(); let apply = false;
  const allowed = action === "list" ? [] : action === "query" ? ["--view", "--definition"] : action === "suggest" ? ["--prompt"] : action === "save" ? ["--definition", "--if-revision"] : ["--file", "--kind", "--line", "--property", "--value", "--if-revision"];
  while (values.length) {
  ...
  const root = action === "suggest" ? flags.get("--dir") : required("--dir");
  let result: unknown;
  if (action === "list") result = listPropertyViews(root!);
    ...
    const suggestion = suggestPropertyView(required("--prompt"));
    result = root ? { ...suggestion, result: queryPropertyView(root, suggestion.definition) } : suggestion;
