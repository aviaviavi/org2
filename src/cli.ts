...
function parseTodoStatusArg(rawStatus: string): TodoStatus | "" {
  ...
  if (token === "done" || token === "complete" || token === "completed" || token === "closed" || token === "resolved") return "done";
  ...
  if (command === "todo") {
    ...
    if (todoAction === "set") {
      if (!todoStatus || (todoStatus !== "todo" && todoStatus !== "in_progress" && todoStatus !== "done" && todoStatus !== "canceled")) {
        console.error("Error: todo set requires --status todo|in_progress|done|canceled (aliases: open, in-progress/in progress/prog, complete/completed/closed/resolved, cancel/cancelled)");
        process.exit(1);
      }
    }
