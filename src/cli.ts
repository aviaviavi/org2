...
const DEFAULT_SYNTAX_HEAD_INCLUDES = [
  ...
  '<script>(function(){var root=document.documentElement;function hasDarkToken(s){return /(^|\\s)(dark|theme-dark|dark-mode)(\\s|$)/.test((s||"").toLowerCase());}function pick(){var explicit=(root.getAttribute("data-theme")||"").toLowerCase();var rootDark=hasDarkToken(root.className||"");var bodyDark=hasDarkToken((document.body&&document.body.className)||"");var dark=(explicit==="dark")||(explicit!=="light"&&(rootDark||bodyDark||window.matchMedia("(prefers-color-scheme: dark)").matches));var l=document.getElementById("org2-prism-light");var d=document.getElementById("org2-prism-dark");if(l&&d){l.disabled=dark;d.disabled=!dark;}}pick();try{window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",pick);}catch(_){};if(typeof MutationObserver!=="undefined"){var mo=new MutationObserver(pick);mo.observe(root,{attributes:true,attributeFilter:["class","data-theme"]});if(document.body){mo.observe(document.body,{attributes:true,attributeFilter:["class","data-theme"]});}else{document.addEventListener("DOMContentLoaded",function(){if(document.body)mo.observe(document.body,{attributes:true,attributeFilter:["class","data-theme"]});pick();},{once:true});}}})();</script>',
