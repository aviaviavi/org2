...
const DEFAULT_SYNTAX_HEAD_INCLUDES = [
  ...
  '<script>(function(){var root=document.documentElement;function pick(){var explicit=(root.getAttribute("data-theme")||"").toLowerCase();var cls=(root.className||"").toLowerCase();var hasDarkClass=/(^|\s)(dark|theme-dark|dark-mode)(\s|$)/.test(cls);var dark=(explicit==="dark")||(explicit!=="light"&&(hasDarkClass||window.matchMedia("(prefers-color-scheme: dark)").matches));var l=document.getElementById("org2-prism-light");var d=document.getElementById("org2-prism-dark");if(l&&d){l.disabled=dark;d.disabled=!dark;}}pick();try{window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",pick);}catch(_){}})();</script>',
