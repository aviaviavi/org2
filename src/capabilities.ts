...
export function buildOrg2CapabilityManifest(): Org2CapabilityManifest {
  return {
    ...
    safety: [
      ...
      "Approval decisions are item-scoped: rejecting or canceling one action leaves sibling approvals pending, and the run resumes only after the current boundary is fully decided, executing approved actions while excluding rejected or canceled ones. Use `revised` with a concrete note when replacement material is required.",
