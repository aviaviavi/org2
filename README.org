...
- HTML export (=org2 export html=): *Available now* (MVP, single-file + directory batch standalone export + optional index page + configurable stylesheet injection + optional table of contents + optional Org file-link rewrite)
...
- Export/publishing pipeline parity: *In progress* (MVP single-file + directory-batch HTML export + index generation + configurable stylesheet injection + optional table of contents + optional Org file-link rewrite + keyword-driven HTML metadata tags available via CLI + VS Code)
...
- Exported HTML now maps =#+AUTHOR=, =#+DATE=, =#+DESCRIPTION=, and =#+KEYWORDS= into corresponding HTML =<meta>= tags.
- Single-file JSON export includes a =metadata= object when those keywords are present.
