...
- Core file structures for daily workflows:
  - headings, drawers/properties, planning lines, TODO keywords
  - links, lists/checkboxes, timestamps, tables (basic usage)
- CLI workflows:
  - agenda
  - TODO set/toggle
  - planning edits (=plan set= and =plan today=)
  - subtree archive
  - IDs/backlinks/query helpers
  - roam utilities (=db-sync=, =node new=, backlink insertion)
  - formatting and LSP server entrypoint
- Editor integrations:
  - VS Code extension (MVP, actively developed)
  - Vim/Neovim plugin (minimal: syntax + folding)

*** What is not complete yet

Most classic Org Mode surface area is still missing or incomplete. In particular, Org2 does *not* yet aim for full parity on areas like:
- rich export/publishing parity,
- Babel execution workflows,
- capture/refile/clocking ecosystems,
- advanced agenda customization from mature Org setups,
- broad editor feature parity beyond current MVP integrations.

If you need stability and broad, long-established Org Mode behavior today, treat Org2 as experimental.
