...
- HTML export (=org2 export html=): *Available now* (MVP, single-file + directory batch standalone export + optional index page)
...
- Export/publishing pipeline parity: *In progress* (MVP single-file + directory-batch HTML export + index generation available via CLI + VS Code)
