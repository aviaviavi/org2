...
- Export/publishing pipeline parity: *In progress* (MVP single-file + directory-batch HTML export + index generation + configurable stylesheet injection + optional/depth-limited table of contents (=--toc-depth= / =#+OPTIONS: toc:N=) + optional Org file-link rewrite + automatic heading anchors [including =:CUSTOM_ID:= targets during file-link rewrite] + keyword-driven HTML metadata tags/lang/head snippets + keyword-driven title/subtitle header rendering available via CLI + VS Code)
