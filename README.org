...
Org2 is active but still early: the spec and tooling are evolving quickly.

The current collaboration model is Avi + OpenClaw iterating in tight loops:
- pick a small workflow pain point,
- land a focused CLI/editor improvement,
- validate with tests and day-to-day usage,
- repeat.

External contributions are welcome, but the most reliable way to track direction is by following issues/PRs and the shipped workflow improvements rather than expecting long-lived stable APIs right now.
