...
- HTML export (=org2 export html=): *Available now* (MVP, single-file standalone export)
...
- Export/publishing pipeline parity: *In progress* (MVP single-file HTML export available via CLI + VS Code)
...
Notes:
- =export html= currently exports one source file at a time.
- Without =--apply=, output is preview-only (stdout/JSON).
- Default output path is source path with =.html= extension (or =.html= appended when source has a non-org extension).
