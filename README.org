...
- LSP go-to-declaration for file/id links (=textDocument/declaration= via =org2 lsp=): *Available now* (MVP)
- LSP go-to-type-definition + go-to-implementation for file/id links (=textDocument/typeDefinition= + =textDocument/implementation= via =org2 lsp=): *Available now* (MVP)
...
- LSP rename for =[[id:...]]= links + =:ID:= properties (=org2 lsp=): *Available now* (MVP)
...
- LSP linked editing ranges for ID links/properties (=textDocument/linkedEditingRange= via =org2 lsp=): *Available now* (MVP)
...
- LSP call hierarchy for Org ID links (=textDocument/prepareCallHierarchy= + =callHierarchy/incomingCalls= + =callHierarchy/outgoingCalls= via =org2 lsp=): *Available now* (MVP)
